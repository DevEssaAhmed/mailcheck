...
def check(email):
    ...
    try:
        process = subprocess.run([str(BINARY), '--', email], capture_output=True,
                                 text=True, encoding='utf-8', errors='replace',
                                 timeout=TIMEOUT, env=env, shell=False)
    except subprocess.TimeoutExpired:
        return {'email': email, 'status': 'unknown', 'reason': 'The check timed out after 60 seconds. The mail server may be unavailable or blocking verification. Check outbound port 25.', 'details': None}
    ...
    reasons = {
        ...
        'unknown': 'Reacher could not confirm this mailbox. This does not mean the address is invalid. The server may block verification, or outbound port 25 may be unavailable.'
    }
    return {'email': email, 'status': status, 'reason': reasons[status], 'details': data,
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if not BINARY.is_file():
            return self.send(503, {'error': 'Reacher is not installed. Stop the app, run python3 setup_reacher.py, then restart it.'})
        ...
        try:
            ...
            self.send(502, {'error': 'Reacher could not run or returned an unreadable result. Run python3 setup_reacher.py and see README.md for troubleshooting.'})
        finally:
            LOCK.release()
